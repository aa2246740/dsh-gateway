...
function parseConfiguredModel(value, reasoningEffort) {
	const selected = value?.trim();
	if (!selected) return void 0;
	const slash = selected.indexOf("/");
	if (slash <= 0 || slash === selected.length - 1) return void 0;
	return {
...
function applyHostReport(before, stamped, session, inbound) {
	...
	switch (report.kind) {
		...
		case "approvalSettled": {
			...
			if (previous) deliveries.push(chat(session.identity, session.key, {
				kind: "approval",
				request: previous,
				handled: true,
...
function applyActorInbound(before, stamped, inbound) {
	...
	const existing = stamped.sessions[key];
	const addressing = inbound.kind === "message" || inbound.kind === "command" ? inbound.addressing : existing?.identity.kind === "group" ? {
		kind: "group",
		mentioned: true,
		botInvited: false
	} : { kind: "dm" };
	...
	if (inbound.kind === "approvalAnswer") {
		const session = working.sessions[key];
		if (!session || session.host.kind !== "bound") return finish(before, working, [], []);
		if (role !== "owner") return finish(before, working, [], [chat(session.identity, session.key, {
			kind: "notice",
			text: "Only the session owner can approve."
		})]);
		...
		return finish(before, putSession(working, {
			...session,
			turn: { kind: "idle" }
		}), [call], [chat(session.identity, session.key, {
			kind: "approval",
			request,
			handled: true,
			answer: inbound.answer
...
function formatModelStatus(pick) {
	if (pick === void 0) return "This session uses the DSH default model from Settings → 模型. Switch with /model provider/model [effort]";
...
async function resolveModelPick(llm, line, current) {
	...
	if (effortOnly) {
		if (!current) return {
			ok: false,
			text: "Select a model first: /model provider/model [effort]"
		};
	...
	if (provider !== void 0) try {
		return {
			...
			pick: compactPick(await llm.resolveCallConfig({
				provider,
				model: parsed.model,
				...parsed.reasoningEffort === void 0 ? {} : { reasoningEffort: parsed.reasoningEffort }
			}))
		};
	} catch (error) {
...
async function findModel(llm, needle, reasoningEffort) {
...
function isGatewayState(raw) {
	...
	return state.version === 1 && Array.isArray(state.seen) && state.seen.every((value) => typeof value === "string") && typeof state.pairingSeq === "number";
}
function isRecord(value) {
	return Boolean(value && typeof value === "object" && !Array.isArray(value));
}
function isPlatformAccess(value) {
	if (!isRecord(value)) return false;
	if (value.kind === "unbound") return true;
	return value.kind === "bound" && typeof value.owner === "string" && Array.isArray(value.allowlist) && Array.isArray(value.guests) && Array.isArray(value.pending);
}
function isMessagingSession(value) {
	...
	return isRecord(value.turn) && typeof value.turn.kind === "string" && Array.isArray(value.queued) && typeof value.lastActivityAt === "number";
}
//#endregion
//#region lib/types/feishu-voice.js
/** Agent-scoped system-prompt section. Not a complete prompt; stock DSH still owns tools and persona. */
const FEISHU_SPEAKING_CONTRACT_SECTION = "dsh-messaging-gateway:feishu-voice";
/**
...
*/
const FEISHU_SPEAKING_CONTRACT = [
...
function visibleChatText(raw) {
	...
	if (PLANNING.test(before) || /^[A-Za-z][\s\S]*[.!?]$/.test(before)) return text.slice(cjk).trimStart();
	return text;
}
/**
...
*/
function splitFirstSpoken(text) {
...
function assistantChunkTextFromEvent(event) {
	...
	for (const record of stream) {
		...
		if (row.type === "text-chunks" && Array.isArray(row.texts)) {
			for (const text of row.texts) if (typeof text === "string" && text.length > 0) parts.push(text);
			continue;
		}
		if (row.type === "chunk" && row.chunk?.type === "text-delta" && typeof row.chunk.text === "string") parts.push(row.chunk.text);
	}
	return parts.join("");
}
...
function installFeishuApprovalHold(agentCtx, hold) {
	const ctx = asHook(agentCtx);
	const listen = () => ctx.on?.("approval/request", ((request, next) => hold(request, next)));
...
var GatewayRuntime = class {
	...
	commitVisibleSent = /* @__PURE__ */ new Set();
	turnRaw = /* @__PURE__ */ new Map();
	sentVisible = /* @__PURE__ */ new Map();
	...
	constructor(args) {
		...
		if (args.selectModel !== void 0) this.selectModel = args.selectModel;
	...
	sendFeishuBubble(hostId, key, text) {
	...
	flushFeishuFirstSpoken(hostId, key, raw) {
	...
	*/
	flushFeishuCommitted(hostId, key, raw) {
	...
	async performInner(call) {
		...
		if (call.kind === "answerApproval") this.ensureApprovalWaiter(String(call.host.hostSessionId)).resolve(call.answer);
	}
	...
	endTurn(hostId, key) {
		...
		this.turnRaw.delete(hostId);
		this.sentVisible.delete(hostId);
		if (text.length > 0 && !alreadySent) this.commit({
			kind: "hostReport",
			sessionKey: key,
			report: {
				kind: "turnProgress",
	...
	modelFor(hostId, key) {
		const session = key ? this.state.sessions[key] : hostId ? this.state.sessions[this.keyForHost(hostId) ?? ""] : void 0;
	...
	agentSetup(pick, opts) {
		...
		if (!pick && this.setupAgent === void 0 && !chatFeel && !feishuCards) return void 0;
		return (agentCtx, agent) => {
			this.setupAgent?.(agentCtx, agent);
			this.installGatewayChatHooks(agentCtx, {
				chatFeel,
				feishuCards
			}, agent);
			const id = agent?.id;
	...
	sessionWantsChatFeel(key) {
		return this.sessionPlatform(key) === "feishu";
	}
	sessionIsFeishu(key) {
		return this.sessionPlatform(key) === "feishu";
	}
	hostWantsChatFeel(hostId) {
	...
	installGatewayChatHooks(agentCtx, opts, agent) {
	...
	async holdFeishuApproval(hostId, request, next) {
		...
		try {
			return await Promise.race([waiter.promise.then(outcomeFromAnswer), next()]);
		} catch {
			return "unavailable";
		}
	}
	...
	setupForAgent(hostId) {
		...
		const key = hostId === void 0 ? void 0 : this.keyForHost(hostId);
	...
	*/
	...
	pinHost(session, created, actualCwd) {
...
//#endregion
//#region lib/types/feishu-card.js
const FEISHU$1 = platformId("feishu");
const FEISHU_CARD_KIND = "dsh-approval";
...
function feishuApprovalCard(request, identity) {
	...
	if (options.includes("allow-once")) actions.push(button("允许一次", "allow-once", identity, requestId, "primary"));
	if (options.includes("deny")) actions.push(button("拒绝", "deny", identity, requestId, "danger"));
	return {
...
function parseValue(raw) {
	...
	const row = value;
	if (row.kind !== "dsh-approval") return void 0;
...
function feishuPostContent(text) {
...
function looksLikeMarkdown(text) {
	return /(^|\n)\s{0,3}#{1,6}\s/.test(text) || /(^|\n)\s*[-*+]\s+\S/.test(text) || /(^|\n)\s*\d+\.\s+\S/.test(text) || /```/.test(text) || /\*\*[^*]+\*\*/.test(text) || /__[^_]+__/.test(text) || /\[[^\]]+\]\([^)]+\)/.test(text) || /(^|\n)\s*\|.+\|/.test(text);
}
function chatTextOutbound(chat, replyId, text) {
...
function feishuOutboundFromDelivery(delivery, cards, live) {
	if (delivery.kind !== "chat" || delivery.identity.platform !== FEISHU) return void 0;
	...
	if (body.kind === "busy") {
		...
		if (body.on) {
			...
			if (!messageId || live.typing.has(chat)) return void 0;
			return {
				chatId: chat,
				msgType: "text",
				content: "",
...
async function presentFeishuDelivery(delivery, say, cards = /* @__PURE__ */ new Map(), live) {
	...
	if (outbound.mode === "react" && live && posted && typeof posted === "object") {
		...
		if (typeof reactionId === "string" && outbound.messageId) live.typing.set(chat, {
...
async function runFeishu(runtime, tokens) {
	...
	const live = {
		lastUserMessageId: /* @__PURE__ */ new Map(),
	...
	const unwatch = runtime.watchDeliveries((deliveries) => {
		for (const delivery of deliveries) {
			...
			presentFeishuDelivery(delivery, async (out) => {
				if (out.mode === "react" && out.messageId) try {
					...
					return reacted.data?.reaction_id ? {
						...
						reactionId: reacted.data.reaction_id
					} : void 0;
				} catch (error) {
					const message = error instanceof Error ? error.message : String(error);
				...
				if (out.mode === "unreact" && out.messageId && out.reactionId) {
					try {
						await client.im.v1.messageReaction.delete({ path: {
							message_id: out.messageId,
							reaction_id: out.reactionId
						} });
					} catch (error) {
						const message = error instanceof Error ? error.message : String(error);
				...
				if (out.replyId && out.replyId.startsWith("om_")) {
					...
					return replied.data?.message_id ? { messageId: replied.data.message_id } : void 0;
				}
				...
				return created.data?.message_id ? { messageId: created.data.message_id } : void 0;
	...
	const started = wsClient.start({ eventDispatcher: new Lark.EventDispatcher({}).register({
...
function resolveMessagingWorkspaceDir(workspaceDir, dshHome = process.env.DSH_HOME ?? join(homedir(), ".dsh")) {
...
function resolvePlatformWorkspaceDir(platform, platformWorkspaceDir, legacyWorkspaceDir, dshHome = process.env.DSH_HOME ?? join(homedir(), ".dsh")) {
...
function isMessagingWorkspaceCwd(cwd, workspaceDir) {
...
//#endregion
//#region lib/types/instance-lease.js
function errorCode(error) {
	return typeof error === "object" && error !== null && typeof error.code === "string" ? error.code : void 0;
}
...
function readOwner(lockPath) {
	try {
		...
		const row = value;
		if (row.version !== 1 || typeof row.token !== "string" || typeof row.pid !== "number" || typeof row.createdAt !== "string" || typeof row.heartbeatAt !== "string") return void 0;
		return row;
	} catch {
		return;
	}
...
function gatewayInstanceLeasePath(statePath = gatewayStatePath()) {
...
function acquireGatewayInstanceLease(statePath = gatewayStatePath(), dependencies = {}) {
	const lockPath = gatewayInstanceLeasePath(statePath);
	const now = dependencies.now ?? Date.now;
	const probePid = dependencies.probePid ?? probeProcess;
	const heartbeatMs = dependencies.heartbeatMs ?? 2e3;
	const staleMs = dependencies.staleMs ?? 1e4;
		...
		try {
			...
			if (owner) {
				...
				if (state !== "dead") return {
					...
					path: lockPath,
					ownerPid: owner.pid,
					reason: state === "alive" ? "active" : "inaccessible"
				};
		...
		return {
			...
			lease: {
				...
				release: () => {
					...
					released = true;
					clearInterval(timer);
					if (readOwner(lockPath)?.token === token) rmSync(lockPath, {
						recursive: true,
						force: true
					});
				}
...
//#endregion
//#region lib/types/session-model.js
/** Read the same durable projection used by the desktop model picker. */
function currentSessionModel(ctx, id) {
	const session = ctx.get("sessions")?.get(SessionId(id));
	if (!session) return void 0;
	const state = ctx.get("sessionProjections")?.stateOf(session, "modelSelection");
	return state?.pending ?? state?.lastUsed ?? void 0;
}
/** Gateway agents can run before the desktop has opened them. No private cache. */
function installSessionModel(ctx, agent) {
	if (agent.id === void 0 || agent.id === null) throw new Error("Messaging model selection requires an Agent scope");
	installModelSelection(ctx, {
		get current() {
			const pick = currentSessionModel(ctx, String(agent.id)) ?? ctx.get("agentDefaultModel")?.currentSelection();
			return pick && {
				provider: pick.provider,
				model: pick.model,
				...pick.reasoningEffort === void 0 ? {} : { reasoningEffort: ReasoningEffortId(pick.reasoningEffort) }
			};
		},
		assembled: void 0
	});
...
function attachWorkspace(ctx, id, cwd, workspaceDir, platform) {
	if (!isMessagingWorkspaceCwd(cwd, workspaceDir)) return;
	const registry = ctx.get("workspaceRegistry");
	if (!registry) return;
	(async () => {
		try {
			let workspace = registry.resolveByPath ? await registry.resolveByPath(workspaceDir) : registry.list?.().find((item) => item.path === workspaceDir);
...
function placeBoundSessions(ctx, runtime, workspaceDirFor, after) {
	const sessions = ctx.get("sessions");
	(async () => {
...
function authorizeHost(req, res, connection) {
	if (!connection) {
...
function confirmOwnerFromSettings(runtime, platform, owner) {
...
function apply(ctx, config) {
	const workspaceDirs = /* @__PURE__ */ new Map();
	const workspaceDirFor = (platform) => {
		const cached = workspaceDirs.get(platform);
		if (cached) return cached;
		const current = source();
		const configured = resolvePlatformWorkspaceDir(platform, platform === "slack" ? current.slackWorkspaceDir : current.feishuWorkspaceDir, current.workspaceDir);
	...
	if (!instance.acquired) {
		const owner = instance.ownerPid === void 0 ? "" : ` pid ${instance.ownerPid}`;
	...
	const registerModel = (commandHost) => commandHost.register({
		...
		description: "Show or switch this session model",
		input: {
			hint: "[provider/model [effort] | effort <level>]",
			attachments: false
		},
		handler: async (invocation) => {
			...
			try {
				return {
					kind: "success",
					text: `${formatModelStatus((await ctx.sessionController.selectModel({
	...
	const setupAgent = (agentCtx, agent) => {
		if (agent?.id === void 0 || agent.id === null) throw new Error("Messaging model selection requires an Agent scope");
	...
	runtime = new GatewayRuntime({
		...
		defaultModel: (platform) => {
			const configured = parseConfiguredModel(platform === "slack" ? source().slackModel : platform === "feishu" ? source().feishuModel : "", platform === "slack" ? source().slackReasoningEffort : source().feishuReasoningEffort);
	...
	const syncSlack = () => {
		...
		const replace = async () => {
			...
			if (!shuttingDown && current.enabled !== false && bot && app && process.env.MESSAGING_GATEWAY_DISABLE_SLACK !== "1") stopSlack = await runSlack(runtime, {
				bot,
				app
			});
		};
	...
	const syncFeishu = () => {
		...
		const replace = async () => {
			...
			if (!shuttingDown && current.enabled !== false && appId && appSecret && process.env.MESSAGING_GATEWAY_DISABLE_FEISHU !== "1") stopFeishu = await runFeishu(runtime, {
				appId,
				appSecret
			});
		};
				...
				json(res, 200, {
					...
					groups: rows.groups.map((group) => ({
						...
						rows: group.rows.map((row) => {
							...
							const live = sessions?.get?.(SessionId(String(row.hostSessionId)));
							const cwd = (sessions?.list?.().find((item) => String(item.id) === String(row.hostSessionId)))?.header?.cwd ?? live?.header?.cwd;
							return cwd ? {
								...row,
								cwd
							} : row;
	...
	ctx.effect(() => {
		...
		return () => {
			shuttingDown = true;
			slackTail.then(async () => {
				await stopSlack?.();
				stopSlack = void 0;
			});
			feishuTail.then(async () => {
				await stopFeishu?.();
				stopFeishu = void 0;
			});
		};
...
//#endregion
export { Config, apply, confirmOwnerFromSettings, inject, name };
